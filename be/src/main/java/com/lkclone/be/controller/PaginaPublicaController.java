...
public class PaginaPublicaController {
    ...
    public PaginaPublicaDTO paginaPublica(@PathVariable String username) {
        Usuario usuario = usuarioService.buscarPorUsername(username);
        usuarioService.registrarVisualizacaoPerfil(usuario);

        List<Link> links = linkService.getLinks(usuario);

        List<LinkResponseDTO> linksDTO = links.stream()
                .map(link -> new LinkResponseDTO(link.getLinkId(), link.getUrl(), link.getLabel(), link.getPictureLink(),
